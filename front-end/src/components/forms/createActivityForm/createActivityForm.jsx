...
const CreateActivityForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (
        activitiesData.name.trim() === '' ||
        activitiesData.image.trim() === '' ||
        activitiesData.hora.trim() === '' ||
        activitiesData.description.trim() === '' ||
        activitiesData.date.trim() === ''
      ) {
        swal("Error", "Todos los campos son obligatorios", "error");
        return;
      }

      const success = await dispatch(createActivities(activitiesData));
      ...
      if (success) {
        ...
        swal("Listo", "Se creó la actividad con éxito", "success");
      } else {
        swal("Error", "No se pudo crear la actividad", "error");
      }
    } catch (error) {
      swal("Error", "No se pudo crear la actividad", "error");
    }
